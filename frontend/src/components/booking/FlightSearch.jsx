...
import { Loader2, Search, ChevronLeft, ChevronRight, ArrowUp, ArrowDown, Plane, Clock, PlusCircle } from 'lucide-react';
import { useBooking } from '../../context/BookingContext'; // Make sure useBooking is imported
...
export default function FlightSearch() {
    // FIX: Get openCartModal from useBooking context
    const { addFlightToCart, openCartModal, cart } = useBooking(); // Make sure cart is also destructured
    ...
    const handleSearchFlights = async () => {
        ...
        try {
            ...
            if (!res.data.flights || res.data.flights.length === 0) {
                ...
                const enhancedFlights = res.data.flights.map(flight => {
                    const defaultSeat = Math.floor(Math.random() * 30) + 1 + (Math.random() > 0.5 ? 'A' : 'B');
                    const defaultGate = 'G' + (Math.floor(Math.random() * 10) + 1);
                    const defaultTerminal = 'T' + (Math.floor(Math.random() * 3) + 1);
                    const defaultClass = Math.random() > 0.7 ? 'Business' : 'Economy';

                    const enhancedLegs = flight.legs.map((leg, idx) => ({
                        ...leg,
                        segmentNumber: idx + 1,
                        seat_number: leg.seat_number || defaultSeat,
                        gate: leg.gate || defaultGate,
                        terminal: leg.terminal || defaultTerminal,
                        flight_class: leg.flight_class || defaultClass,
                        isTransit: idx < flight.legs.length - 1,
                        transitDurationMinutes: idx < flight.legs.length - 1 ? (Math.floor(Math.random() * 90) + 60) : null
                    }));
    ...
    // FIX: Call both addFlightToCart and openCartModal
    const handleAddFlight = (flight) => {
        addFlightToCart(flight);
        // Ensure cart.length is checked *after* addFlightToCart potentially adds it
        // Since setCart is async, cart here might be stale.
        // A direct call to openCartModal is simpler.
        openCartModal(); // Call the stable openCartModal
    };
