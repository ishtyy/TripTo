...
export const BookingProvider = ({ children }) => {
      ...
      try {
        ...
        if (itineraryResponse.data) {
          ...
          if (
            ...
            Object.keys(initialPassengers).length === fetchedCart.length
          ) {
            setBookingStep("review");
            setCurrentFlightIndex(0);
          } else if (fetchedCart.length > 0) {
            setBookingStep("details");
            const firstFlightWithoutDetailsIndex = fetchedCart.findIndex(
              (f) => !initialPassengers[f.id]
            );
            setCurrentFlightIndex(
              firstFlightWithoutDetailsIndex !== -1
                ? firstFlightWithoutDetailsIndex
                : 0
            );
          }
        }
        ...
        const comprehensiveResponse = await api.get(
          "/bookings/itinerary/comprehensive"
        );
  ...
  const addHotelToCart = useCallback(
    async (hotelBooking) => {
      try {
        // Check if hotel booking already exists
        if (
          cartItems.find(
            (item) => item.type === "hotel" && item.id === hotelBooking.id
          )
        ) {
  ...
  const addPackageToCart = useCallback(
    async (packageBooking) => {
      try {
        // Check if package booking already exists
        if (
          cartItems.find(
            (item) => item.type === "package" && item.id === packageBooking.id
          )
        ) {
        ...
        const cartItem = {
          ...
          data: {
            ...packageBooking,
            // Mark as simple cart item (no detailed booking required)
            isCartOnly: true,
            requiresBookingDetails: false,
          },
        ...
        setCartItems((prevItems) => {
          const newItems = [...prevItems, cartItem];
          if (prevItems.length === 0) {
            // For cart-only items, open cart view instead of details
            openBookingModal("cart");
          }
          return newItems;
        });

        // Don't show the detailed booking flow for cart items
        // User can proceed to review and payment directly
      } catch (error) {
  ...
  const addFlightToCart = useCallback(
    async (flight) => {
      try {
        if (
          cartItems.find(
            (item) => item.type === "flight" && item.id === flight.id
          )
        ) {
    ...
    (itemId) => {
      setCartItems((prevItems) => {
        ...
        if (removedItem && removedItem.type === "flight") {
          setCart((prevCart) =>
            prevCart.filter((flight) => flight.id !== itemId)
          );
  ...
  const removeFlightFromCart = useCallback(
    async (flightId) => {
      try {
        ...
        setCart((prevCart) => {
          ...
          setCurrentFlightIndex((prevIndex) => {
            const newIndex = Math.min(
              prevIndex,
              updatedCart.length > 0 ? updatedCart.length - 1 : 0
            );
            return newIndex;
          });
          return updatedCart;
        ...
        console.error(
          "Failed to remove flight from persistent itinerary:",
          error.response?.data || error
        );
        toast.error(
          error.response?.data?.message ||
            "Failed to remove flight from itinerary."
        );
      }
    },
  ...
  const addPassengerAndSeatInfo = useCallback(
    async (flightId, passengerData, seatNumber) => {
      if (!activeItineraryId) {
        ...
        try {
          ...
          if (response.data && response.data.itineraryId) {
            ...
            toast.error(
              "No active itinerary found. Please add flights to your cart first."
            );
            console.error(
              "Itinerary ID is missing for update details. No active itinerary found."
            );
            return;
          }
        } catch (error) {
          ...
          console.error(
            "Itinerary ID is missing for update details. Failed to fetch itinerary:",
            error
          );
          return;
        }
      ...
      try {
        ...
        const itineraryIdToUse =
          activeItineraryId ||
          (await api.get("/bookings/itinerary")).data?.itineraryId;
        ...
        if (nextIndex < cart.length) {
          ...
          toast.info(
            `Proceeding to next flight (${nextIndex + 1} of ${cart.length}).`
          );
        } else {
          setBookingStep("review");
          setCurrentFlightIndex(0);
        ...
        console.error(
          "Failed to save passenger/seat details persistently:",
          error.response?.data || error
        );
        toast.error(
          error.response?.data?.message ||
            "Failed to save passenger/seat details."
        );
      }
    },
  ...
  const confirmBooking = useCallback(async () => {
    try {
      ...
      for (const item of cartItems) {
        if (item.type === "flight") {
          ...
          bookingPromises.push(
            api
              .post("/bookings", flightBookingData)
          ...
          bookingPromises.push(
            api
              .post("/hotels/book", hotelBookingData)
          ...
          bookingPromises.push(
            api
              .post("/packages/book", packageBookingData)
      ...
      toast.success(
        `${successfulBookings.length} booking(s) confirmed successfully!`
      );
